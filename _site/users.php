...
$pg_qlrs = pg_query ($pg, "SELECT id, firstname, patronymic, surname, email, phone, (SELECT SUM(nodes.balance) * SUM(nodes.repay) FROM nodes WHERE nodes.userid = users.id) FROM users;");
print ("<table cellpadding='3' cellspacing='0' border='1'>");
print ("<tr>\n");
print ("<th>#</th><th>Имя</th><th>Отчество</th><th>Фамилия</th><th>e-Mail</th><th>Телефон</th><th>Ежемесечная выплата</th>");
print ("</tr>\n");
